package com.example.ascendio.presentation.vegetable
...
fun VegetableScreen(
    ...
    val uiState by vegetableViewModel.uiState.collectAsState()
    ...
    Scaffold(
        ...
        Column(modifier = Modifier
            .padding(it)
            .fillMaxSize()) {
            Chocolates.vegetableList.forEach {eachItem ->
                ChocolateItem(uiState = uiState, addItem = {item ->
                    vegetableViewModel.addItem(item)
                }, removeItem = {item ->
                    vegetableViewModel.removeItem(item)
                }, chocolate = eachItem)
            }
